...
/**
 * @breif A process control block (PCB) Process control block (PCB) is a data structure representing a process in the system. 
 * A process should have at least an ID and a state(i.e. NEW, READY, WAITING or TERMINATED). 
 ...
 * 
 * This method is typically called when a PCB is added to or removed from the ReadyQueue:
 * -when added: state changes to READY
 * -when removed for execution: staet changes to RUNNING 
 * 
